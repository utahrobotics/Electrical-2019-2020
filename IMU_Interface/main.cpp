...
void clk_ISR(void) {
    ...
    if ((idx == 0 && bitCounter < 5) || ((derBuffer & 0x3f) ^ 0x3e)) {
        ...
        if (++bitCounter == 16) {
            ...
            Serial1.print(idx-1);
            Serial1.print("]=0x");
            Serial1.println(buffer[idx-1], HEX);
#endif
        }
    }
#ifdef PRINT_TIMES
    t1 = nanos();
    Serial1.print("clk_ISR ns: ");
    Serial1.println(t1-t0, 2);
#endif
}
...
#ifdef DEBUG_ROS
    imu_msg.header.stamp.fromSec(seconds());
    imu_msg.angular_velocity.x = (float) buffer[0];
    imu_msg.angular_velocity.y = (float) buffer[1];
    imu_msg.angular_velocity.z = (float) buffer[2];
    imu_msg.linear_acceleration.x = (float) buffer[3];
    imu_msg.linear_acceleration.y = (float) buffer[4];
    imu_msg.linear_acceleration.z = (float) buffer[5];
    imu_msg.orientation.w = (float) buffer[12];
#else
...
void loop() {
    if (ready) {
        ...
        for (size_t i = 0; i < MAX_HDLC_FRAME_LENGTH; i++) {
            if (buffer[i] < 16) {
                Serial1.print("000");
            }
